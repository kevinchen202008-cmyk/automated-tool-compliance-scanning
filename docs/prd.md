...
- FR-REPORT-01：每个工具生成一份结构化报告，至少包含以下信息（与当前实现对齐）：
  - 工具名称、工具版本（如可获取）
  - 使用许可/开源协议信息（类型、版本、模式）
  - 公司信息（公司名称、所属国家、总部、是否有中国分公司或服务）
  - 商用用户使用限制（是否需购买 License、是否允许免费商用、限制说明、用户/功能限制）
  - 可替代方案列表（优先推荐免费开源工具，包含推荐理由和适用场景）
  - 数据来源与工具信息库更新建议（如 `data_source` 与 `knowledge_base_update`，用于标识本次 AI / 工具信息库 / 混合以及是否建议入库/更新）
  - （可选、后续扩展）综合合规性评分、合规性建议文本、合规性参考链接或标准名称、不合规原因列表等
